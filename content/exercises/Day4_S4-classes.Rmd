...
A `colData` is just a `data.frame` (or `DataFrame`) with one row per sample.
For a single sample, it has a single row. Retrieve the installed file path with
`system.file()`.
...
colData <- data.frame(
    file = bw_file,
    # TODO: any other useful sample-level metadata (sample, type, target, ...)
)
```
...
This is exactly the `distance` column you built in `computeCoverage()` in Day 1:
`seq(-width/2, width/2 - 1, by = 1)`.

```{r}
width <- 2000
rowData <- data.frame(
    distance = seq(______, ______, by = 1)
)
```
...
Run your Day 1 pipeline up to `computeCoverage()` to obtain the `mean`,
`ci_high` and `ci_low` vectors, then wrap each into a one-column matrix with
`matrix(..., ncol = 1)`.

```{r}
bed_file <- system.file("extdata", "Scc1-peaks.narrowPeak", package = "JacquesTestPackage")
l <- importFiles(bw_file, bed_file, width = width) |> filterGRanges()
df <- computeCoverage(l)
assays <- list(
    'mean'  = matrix(df$mean, ncol = 1),
    'upCI'  = matrix(______, ncol = 1),
    'lowCI' = matrix(______, ncol = 1)
)
```
...
Use `methods::setClass()`. `contains =` declares the parent class to inherit
from, and `slots =` is a named list mapping each new slot to the class it holds.
...
`methods::new("AggregatedCoverage", <a SummarizedExperiment>)` creates the
object with its two extra slots left empty; you then fill them with `@`.

```{r}
AC <- methods::new(
    "AggregatedCoverage",
    SummarizedExperiment::SummarizedExperiment(
        rowData = ______, colData = ______, assays = ______
    )
)
AC@features <- ______   # the GRanges of features
AC@width <- ______      # the width, as an integer
```
...
Simply move all the steps you just ran by hand (build `colData`, `rowData`,
`assays`, the `SummarizedExperiment`, then the two extra slots) into the body of
a single function, returning the final object.

```{r}
AggregatedCoverage <- function(bw_file, features_file, width) {
    # TODO: colData, rowData
    # TODO: l <- importFiles(...) |> filterGRanges(); df <- computeCoverage(l)
    # TODO: assays
    # TODO: AC <- methods::new("AggregatedCoverage", SummarizedExperiment(...))
    # TODO: AC@features <- ...; AC@width <- as.integer(width)
    return(AC)
}
```
...
`setMethod("plot", "AggregatedCoverage", function(x, ...) { ... })` registers a
`plot` method for your class. Inside, pull the data back out of the object with
`assay(x, "mean")`, `assay(x, "upCI")`, `assay(x, "lowCI")` and
`rowData(x)$distance`, assemble a `data.frame`, and hand it to `ggplot()`.

```{r}
setMethod("plot", "AggregatedCoverage", function(x, ...) {
    # TODO: build a data.frame from rowData(x) and the three assays
    # TODO: ggplot(...) + geom_line() + geom_ribbon()
})
```
