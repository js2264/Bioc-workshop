...
- Document a new S4 class and its constructor; 
...
1. a named list of `.RleList` objects (`RleList` are `bigwig` files imported in `R`);
2. a `GRanges` object (containing genomic features of interest)
...
colData <- data.frame(
    file = c(
        here::here('../../../Share/Day4/Scc1-calibrated_rep1.bw'),
        here::here('../../../Share/Day4/Scc1-calibrated_rep2.bw')
    )
)
```
